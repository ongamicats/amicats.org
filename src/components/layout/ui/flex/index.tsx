...
export function Flex({
  ...
  if (stackAt && direction !== 'col') {
    ...
    directionClass = cn(
      baseCol,
      reverse ? stackAtReverseClasses[stackAt] : stackAtRowClasses[stackAt],
    );
  } else {
    directionClass =
      direction === 'col'
  ...
  if (mobileAlign && align) {
    alignClass = cn(
      mobileAlignClasses[mobileAlign],
      desktopAlignClasses[align],
    );
  } else if (mobileAlign) {
  ...
  if (mobileJustify && justify) {
    justifyClass = cn(
      mobileJustifyClasses[mobileJustify],
      desktopJustifyClasses[justify],
    );
  ...
  const growClass = growAt
    ? growAtClasses[growAt]
    : grow
      ? 'flex-1 min-w-0'
      : '';
