...
import { LandingNavbar as Navbar } from '@/routes/landing/-components/landing-navbar'
...
describe('LandingNavbar (unit) — scroll-threshold behavior', () => {
  ...
  test('2) navbar becomes visible when window.scrollY >= 24 and shows new UI elements', async () => {
    ...
    await waitFor(() => {
      ...
      // Accept either menuitem or option role depending on implementation
      let menuItems = screen.queryAllByRole('menuitem')
      if (menuItems.length === 0) menuItems = screen.getAllByRole('option')
